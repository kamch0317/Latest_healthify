...
class UpdateFragment : Fragment(), AdapterView.OnItemSelectedListener {
    ...
    lateinit var updateSlider2: Slider
    ...
    override fun onCreateView(
        ...
        (activity as AppCompatActivity).supportActionBar?.title = "Update Workout"

        updateSlider = view.findViewById(R.id.update_slider)
        updateSlider2 = view.findViewById(R.id.update_slider_sets)
        ...
        view.update_slider_value.text = args.currentWorkout.workoutReps
        view.update_slider_value2.text = args.currentWorkout.workoutSets
    ...
    fun sliderFunct2() {
        ...
        updateSlider2.addOnSliderTouchListener(object : Slider.OnSliderTouchListener {
            @SuppressLint("RestrictedApi")
            override fun onStartTrackingTouch(slider2: Slider) {
                val slideValue2 = slider2.value

                Log.i("Slider Value", df.format(slideValue2))
            }

            @SuppressLint("RestrictedApi")
            override fun onStopTrackingTouch(slider2: Slider) {
                val value2 = slider2.value

                view!!.update_slider_value2.text = df.format(value2)

                slider2.addOnChangeListener { slider2, _, _ ->
                    val value2 = slider2.value
                    view?.update_slider_value2!!.text = df.format(value2)
                }
            }
    ...
    private fun updateWorkout() {
        ...
        val work5 = update_slider_value2.text.toString()

        if (inputCheck(workName, workDes)) {
            val updatedWork = Workout2(args.currentWorkout.id, workName, workDes, work3, work4, work5)
